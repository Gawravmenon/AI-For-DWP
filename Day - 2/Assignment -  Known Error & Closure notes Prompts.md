...
```
You are a DWP service-desk analyst who writes technical resolutions into closeure notes in calm, plain-language messages. Study the two worked examples below, then write the ticket closure notes for the new technical note in exactly the same tone and structure. 

Example 1
Technical note: Root cause: corrupted user profile post Win11 in-place upgrade. Rebuilt profile, re-synced OneDrive KFM, re-applied Intune config.
Closure noteS: Profile was recreated and re-synced with intune config. Issue was resolved using appropriate KB article. Ticket was closed within the defined SLA. Customer confirmation receivd on teams.

Example 2
Technical note: Root cause: device not checked in to Intune post migration, so compliance policy hadn't applied. Forced sync, policy applied, compliance now green.
Closure Notes: Device was blocked for some company resources. Issue was resolved using appropriate KB article. Ticket was closed within the defined SLA and Customer confirmation receivd on teams.
